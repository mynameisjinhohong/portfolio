...
public class Ai1Manager_HJH : MonoBehaviour
{
    ...
    void Start()
    {
        if (StageMenu_lyd.instance.whatCountry == BTNType.Korea || StageMenu_lyd.instance.whatCountry == BTNType.Japan)
        {
            for (int i = 0; i < 4; i++)
            {
                int x = Random.Range(0, 7);
                int y = Random.Range(35, 67);
            ...
            for (int i = 0; i < 4; i++)
            {
                int x = Random.Range(8, 38);
                int y = Random.Range(52, 67);
                StartCoroutine(MakeEnemyPalace(x, y, 6));
                StartCoroutine(MakeEnemySuwon(x + 1, y));
                StartCoroutine(MakeEnemyArmy(x, y + 1));
